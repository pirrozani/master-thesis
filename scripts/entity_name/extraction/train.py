"""Training script for entity name extraction model."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Train entity name extraction model with QLoRA',
...
Examples:
  uv run scripts/entity_name/extraction/train.py data/processed/entity_name
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Output directory for adapter weights (default: from model config)',
    )
    parser.add_argument(
        '--num-epochs',
        type=int,
    ...
    # Validate dataset path is provided for training
    if not args.dataset_path:
        parser.error('dataset_path is required for training')
    ...
    output_dir = args.output_dir or config.adapter_dir
    ...
    trainer = EntityNameModelTrainer(
        model=config,
    ...
    try:
        ...
        if 'out of memory' in str(e).lower():
            ...
            print('  - Reduce max_seq_length (currently 512)')
    ...
    print('Use these weights for inference with EntityNameExtractor')
    print('=' * 60)
